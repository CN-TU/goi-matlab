...
if length(dflag)>1
   if length(dflag)~=6, error('Variable mismatch error'); end
else
    dflag=dflag*ones(1,6);
end  
...
for i=1:k
    ...
    if mv(i)==1
        ...
        for j=1:N
            ...
            switch d(j,i)
                ...
                otherwise, pd = makedist('Normal','mu',0,'sigma',cp(i)); %Normal
            end
    ...
    % -------------- Placing clusters in the output space (step 1)
    ...
    % -------------- Placing clusters in the output space (step 2)
    %Ni = bsxfun(@plus, Ni,c(i,:)); 
    MN=[MN;Ni];
...
r.perf.Silhouette=mean(silhouette(MN(label>0,:),label(label>0),'Euclidean'));
